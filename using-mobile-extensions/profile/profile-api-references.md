...
```swift
public static func removeUserAttributes(attributeNames: [String])
```
...
```swift
UserProfile.removeUserAttributes(["username","usertype"]);
```
{% endtab %}
{% endtabs %}
...
```objectivec
[AEPMobileUserProfile getUserAttributes:attributes completion:^(NSDictionary* dict, NSError* error){
    // your customized code
    }];
```
