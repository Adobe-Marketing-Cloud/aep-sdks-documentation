...
* Calling the Core `trackAction` or `trackState` public API.
...
* Calling the Analytics `sendQueuedHits` public API
* Calling the Analytics `clearQueue` public API
* Calling the Analytics `getQueueSize` public API
...
This event will be generated the Analytics `getTrackingIdentifier` and `getCustomVisitorIdentifier` public API call.
...
This event is generated as a paired response for the `getTrackingIdentifier` and `getCustomVisitorIdentifier` public API that creates an _com.adobe.eventType.analytics_ event type and a _com.adobe.eventSource.requestIdentity_ event source.
