...
| `a.message.triggered` | String \("1"\) | No | Flag that specifies a message triggered event. Note, only one type of message interaction is present in each campaign response content event. |
