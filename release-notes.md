...
**iOS Core 2.3.4**
* Fixed a crash that may happend during app shutdown.
* Fixed a bug where SDK does not functioning in the race condition when SDK is being initiliazed in multiple thread.
* Fixed a bug where the downloaded rules zip file may not be decompressed.
* Fixed a bug where the `getSdkIdentities` API used wrong key names for Push ID and IDFA.
